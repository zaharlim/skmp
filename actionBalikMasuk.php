...
$pid = $_POST['id'];
	...
	$query = "UPDATE `balik_kampung` SET `masamasuk`=now(), `tarikhmasuk` = '$tarikh' WHERE `pid`='$pid'";
	...
	$query1 = "UPDATE `balik_kampung` SET `masamasuk`=now(), `tarikhmasuk` = '$tarikh' WHERE `pid`='$pid'";
	$result1 = mysqli_query($conn, $query1) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($conn), E_USER_ERROR);
	
	$query2 = "INSERT INTO `pulanglewat` (`fid`,`masamasuk`) VALUES ('$pid',now())";
